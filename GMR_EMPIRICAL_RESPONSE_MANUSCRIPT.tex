...
The null model is not that all residuals vanish.  Real rotation curves contain distance errors, inclination uncertainties, stellar mass-to-light uncertainty, non-circular motions, gas corrections, and environmental effects \citep{Trachternach2008,BellDeJong2001,Chabrier2003,Meidt2014,Courteau2014}.  In addition, residuals can be introduced or reshaped by the kinematic modelling used to extract circular velocities from data cubes or velocity fields, including tilted-ring and three-dimensional fitting approaches such as ROTCUR, $^{3{\rm D}}$Barolo, and TiRiFiC \citep{Begeman1991,DiTeodoro2015,Jozsa2007,Iorio2017}.  The stricter and testable null is therefore conditional: after a flexible one-variable acceleration law and the adopted rotation-curve extraction pipeline have accounted for $\log g_{\rm bar}$, the remaining residuals are expected not to show an additional systematic dependence on baryonic structural invariants.  That null is modeled with a cubic RAR baseline,
\begin{equation}
